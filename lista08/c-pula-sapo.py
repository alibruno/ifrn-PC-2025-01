...
A entrada é dada em duas linhas. A primeira tem dois inteiros positivos P e N, a 
altura do pulo do sapo e o número de canos (1 ≤ P ≤ 5 e 2 ≤ N ≤ 100). A segunda 
linha tem N inteiros positivos que indicam as alturas dos canos ordenados da esquerda 
para a direita. Não há altura maior do que 10.

Output

A saída é dada em uma única linha. Se o sapo pode chegar no cano mais à direita, 
escreva "YOU WIN". Se o sapo não consegue, escreva "GAME OVER".
'''
...
for i in range(N-1):
    if (listaN[i] - listaN[i+1] > P or listaN[i+1] - listaN[i] > P):
        print("GAME OVER")
        break
    if (i == N - 2):
        print("YOU WIN")